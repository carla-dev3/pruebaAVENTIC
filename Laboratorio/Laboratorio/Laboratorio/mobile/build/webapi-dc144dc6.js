...
const { Storage: Storage$1 } = Plugins;
var storage;
(function (storage) {
    ...
    async function get(key) {
        const item = await Storage$1.get({ key: key });
        return JSON.parse(item.value);
    }
    storage.get = get;
    async function remove(key) {
        await Storage$1.remove({
            key: key
        });
    }
    storage.remove = remove;
    async function keys() {
        return (await Storage$1.keys()).keys;
    }
    storage.keys = keys;
    async function clear() {
        await Storage$1.clear();
    }
    storage.clear = clear;
})(storage || (storage = {}));
...
}(typeof window !== "undefined" ? window : undefined, function () { 'use strict';
...
class SyncOptions {
    constructor() {
        this.partialSync = false;
        this.syncTables = [];
        this.syncViews = [];
        this.partialSend = false;
        this.sendTables = [];
        this.sendViews = [];
...
class compareData {
    constructor(tableName, fields, keyFields) {
        this.tableName = tableName;
        this.fields = fields;
...
class Webapi {
    ...
    getAuth() {
        return storage$1.get('flexyAuth');
    }
    async saveAuth(auth) {
        storage$1.set('flexyAuth', auth);
    }
    async disconnect() {
...
export { Capacitor$1 as C, DependencyConfig as D, FilesystemDirectory as F, GroupConfig as G, MenuConfig as M, ObjectConfig as O, Plugins as P, RelationConfig as R, ScriptConfig as S, UserProfileConfig as U, ViewConfig as V, Webapi as W, __extends as _, storage$1 as a, ConfToken as b, FileConfig as c, RelationKey as d, PageConfig as e, StyleConfig as f, fileResource as g, fieldConfig as h, PropertyConfig as i, syncObj as j, syncResult as k, compareData as l, __decorate as m, Camera as n, CameraResultType as o, CameraSource as p, storage as s };
